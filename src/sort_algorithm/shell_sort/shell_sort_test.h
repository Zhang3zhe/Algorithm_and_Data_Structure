 ...
 */
#ifndef SHELL_SORT_TEST_H_
#define SHELL_SORT_TEST_H_
#include<iostream>
#include<vector>
#include<algorithm>
...
class ShellSortTest {
 public:
	 static void TEST() {
		 std::cout << "----- ShellSortTest: -----" << std::endl;
		 ...
		 std::cout << "----- ShellSortTest over, access rate is " << 100.0 * access_no / case_no << "%. -----" << std::endl;
	 }
 ...
 private:
	 ...
	 static void judge(std::vector<int>& lhs, std::vector<int>& rhs) {
		 ads::shell_sort(lhs.begin(), lhs.end());
...
int ShellSortTest::case_no = 0;
int ShellSortTest::access_no = 0;

#endif // SHELL_SORT_TEST_H_
