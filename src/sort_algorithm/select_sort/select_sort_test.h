 ...
 */
#ifndef SELECT_SORT_TEST_H_
#define SELECT_SORT_TEST_H_
#include<iostream>
#include<vector>
#include<list>
#include<algorithm>
...
class SelectSortTest {
 public:
	 static void TEST() {
		 std::cout << "----- SelectSortTest: -----" << std::endl;
		 ...
		 std::cout << "----- SelectSortTest over, access rate is " << 100.0 * access_no / case_no << "%. -----" << std::endl;
	 }
 ...
 private:
	 ...
	 static void judge(std::vector<int>& lhs, std::vector<int>& rhs) {
		 ads::select_sort(lhs.begin(), lhs.end());
...
int SelectSortTest::case_no = 0;
int SelectSortTest::access_no = 0;

#endif // SELECT_SORT_TEST_H_
