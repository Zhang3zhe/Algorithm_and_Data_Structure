...
class RadixSortTest {
 public:
	 static void TEST() {
		 std::cout << "----- RadixSortTest: -----" << std::endl;

		 // 4组std::vector： 增序排列、降序排列、有重复数字、单元素的std::vector
		 std::vector<int> data, cmp_data;

		 data = { 123, 234, 345, 56, 567, 678, 79, 890, 901, 912 };
		 cmp_data = { 123, 234, 345, 56, 567, 678, 79, 890, 901, 912 };
		 judge(data, cmp_data);
		 ...
		 std::cout << "----- RadixSortTest over, access rate is " << 100.0 * access_no / case_no << "%. -----" << std::endl;
	 }
 ...
 private:
	 ...
	 static void judge(std::vector<int>& lhs, std::vector<int>& rhs) {
		 ads::radix_sort(lhs.begin(), lhs.end(), 3);
...
int RadixSortTest::case_no = 0;
int RadixSortTest::access_no = 0;
