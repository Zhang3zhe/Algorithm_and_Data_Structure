 ...
 */
#ifndef COUNT_SORT_TEST_H_
#define COUNT_SORT_TEST_H_
#include<iostream>
#include<vector>
#include<algorithm>
...
class CountSortTest {
 public:
	 static void TEST() {
		 std::cout << "----- CountSortTest: -----" << std::endl;
		 ...
		 std::cout << "----- CountSortTest over, access rate is " << 100.0 * access_no / case_no << "%. -----" << std::endl;
	 }
 ...
 private:
	 ...
	 static void judge(std::vector<int>& lhs, std::vector<int>& rhs) {
		 ads::count_sort(lhs.begin(), lhs.end(), 20);
...
int CountSortTest::case_no = 0;
int CountSortTest::access_no = 0;

#endif // COUNT_SORT_TEST_H_
